...
, { formatValue, formatError } = require('sh.orchestration-tools');
 ...
 * @returns {string}
 */
const defaultValueFormatter = val => formatValue(val);

/**
 * Formats an Error like this if err is (all returned values are prefixed by 'Error: '):
 * - undefined or null: empty string
 * - not an Error: uses defaultValueFormatter(err)
 * - an Error: formatError(err) from orchestration-tools
 * 
 * @param {Error|any} err
 * @returns {string}
 */
...
/**
 * Uses the defaultValueFormatter and the defaultErrorFormatter to create a string-representation of the given state and/or error. Note that this function can be used to format only one value, by providing the respective default value for the other. If both state and error are given, creates a string with these separated by a space.
 * 
...
const defaultEventFormatter = (eventId = 0) => {
  ...
  return typeof eventId === 'number' ? eventId.toString() :
...
class BaseLogger extends EventEmitter {
  ...
  constructor(typeOrClassOrCtorFunc) {
    ...
    if (typeof typeOrClassOrCtorFunc !== 'function' && typeof typeOrClassOrCtorFunc !== 'string') {
      throw new Error(`The given type, class or constructor is not a string or (constructor-) function: '${inspect(typeOrClassOrCtorFunc)}'.`);
    }
